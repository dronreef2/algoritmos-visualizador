        ...
        if not client.is_configured():
            ...
            with col1:
                query_personalizada = st.text_input(
                    "🔍 Ou digite sua própria busca:", placeholder="Ex: algoritmo de ordenação merge sort explicação"
                )

            with col2:
                profundidade = st.selectbox(
                    "🎯 Profundidade:", ["basic", "advanced"], help="Basic: busca rápida | Advanced: busca mais profunda"
                )
                        ...
                        if "error" in resultado:
                            ...
                            if "results" in resultado and resultado["results"]:
                                ...
                                for i, item in enumerate(resultado["results"], 1):
                                    with st.expander(
                                        f"📄 Resultado {i}: {item.get('title', 'Título não disponível')[:60]}..."
                                    ):
                                        ...
                                        if st.button(
                                            f"🔍 Buscar mais sobre '{item.get('title', '')[:30]}...'", key=f"related_{i}"
                                        ):
            ...
            if query_final and resultado and "results" in resultado:
                st.session_state.search_history.append(
                    {"query": query_final, "timestamp": time.time(), "results_count": len(resultado.get("results", []))}
                )
