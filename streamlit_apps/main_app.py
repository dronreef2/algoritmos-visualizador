...
def setup_page_config():
    ...
    st.set_page_config(
        page_title="🎯 Algoritmos Visualizador", page_icon="🎯", layout="wide", initial_sidebar_state="expanded"
    )
    ...
    try:
        ...
        with col2:
            ...
            max_results = st.slider(
                "Máximo de resultados:", min_value=1, max_value=10, value=5, help="Número máximo de resultados a retornar"
            )
                            ...
                            for i, item in enumerate(result["results"], 1):
                                with st.expander(f"📄 Resultado {i}: {item.get('title', 'Sem título')[:50]}..."):
                                    ...
                                    if item.get("content"):
                                        ...
                                        st.text_area("Conteúdo", item["content"], height=150, key=f"content_{i}")
                        else:
...
def render_home_page():
    ...
    **progresso personalizado** com visualizações interativas.
    ...
    if APRENDIZADO_CONTEXTUAL_DISPONIVEL:
        st.info(
            "🎯 **Novo:** Sistema de Aprendizado Contextualizado disponível! Explore jornadas temáticas e conexões entre conceitos."
        )
...
def render_module_1():
    ...
    try:
        from modulo_1_fundamentos.aplicacoes_reais import SistemaBuscaLogs, DetectorFraudesFinanceiras, RedeSocialBFS
...
def render_social_network():
    ...
    try:
        ...
        try:
            ...
            nx.draw(G, pos, with_labels=True, node_color="lightblue", node_size=1500, font_size=10, font_weight="bold", ax=ax)
...
def render_binary_search():
    ...
    def binary_search_steps(arr, target):
        ...
        while left <= right:
            mid = (left + right) // 2
            steps.append({"left": left, "right": right, "mid": mid, "comparison": arr[mid], "found": arr[mid] == target})
...
def render_two_pointers():
    ...
    problema = st.selectbox(
        "Escolha o Problema", ["🎯 Two Sum (Array Ordenado)", "🔄 Verificar Palíndromo", "🎨 Container with Most Water"]
    )
...
def render_palindrome_check():
    ...
    def palindrome_steps(s):
        ...
        while left < right:
            steps.append(
                {"left": left, "right": right, "left_char": s[left], "right_char": s[right], "match": s[left] == s[right]}
            )
...
def render_container_water():
    ...
    if steps:
        ...
        # Área do container
        ax.fill_between([left_idx, right_idx], 0, container_height, alpha=0.3, color="blue", label="Container")
...
def render_sliding_window():
    ...
    problema = st.selectbox(
        "Escolha o Problema",
        ["📊 Soma Máxima de Subarray (Tamanho K)", "🔍 Substring mais longa sem repetição", "🎯 Mínimo em Janela Deslizante"],
    )
...
def render_max_sum_subarray():
    ...
    def sliding_window_max_sum(arr, k):
        ...
        steps.append({"start": 0, "end": k - 1, "window": arr[:k].tolist(), "sum": window_sum, "max_sum": max_sum})
        ...
        for i in range(1, len(arr) - k + 1):
            ...
            steps.append(
                {"start": i, "end": i + k - 1, "window": arr[i : i + k].tolist(), "sum": window_sum, "max_sum": max_sum}
            )
    ...
    if steps:
        ...
        # Destacar janela
        ax.axvspan(current_step["start"] - 0.5, current_step["end"] + 0.5, alpha=0.3, color="yellow", label="Janela")
...
def render_graph_structure():
    ...
    try:
        ...
        try:
            ...
            fig, ax = plt.subplots(figsize=(10, 8))
            pos = nx.spring_layout(G)
            nx.draw(G, pos, with_labels=True, node_color="lightblue", node_size=1500, font_size=12, font_weight="bold", ax=ax)
...
def render_module_3():
    ...
    try:
        ...
        # Seleção de problema
        problema = st.selectbox(
            "Escolha o Problema", ["🔢 Fibonacci", "🎒 Knapsack (0/1)", "📝 Longest Common Subsequence", "🪙 Coin Change"]
        )
...
def render_fibonacci_problem():
    ...
    try:
        ...
        # Abordagem
        abordagem = st.radio("Escolha a Abordagem", ["💪 Força Bruta", "🧠 Memoização", "📊 Tabulação"])
...
def render_knapsack_problem():
    ...
    try:
        ...
        for i, (w, v) in enumerate(zip(weights, values)):
            ax1.annotate(f"Item {i+1}", (w, v), xytext=(5, 5), textcoords="offset points", fontsize=8)
        ax1.set_xlabel("Peso")
...
def render_coin_change_problem():
    ...
    try:
        ...
        for bar, coin in zip(bars, coins):
            ...
            ax.text(bar.get_x() + bar.get_width() / 2.0, height, f"{coin}", ha="center", va="bottom", fontweight="bold")
...
def render_module_4():
    ...
    try:
        ...
        mode = st.selectbox(
            "Escolha o Modo",
            ["🎯 Simulação de Entrevista", "📚 Biblioteca de Problemas", "📊 Análise de Código", "⏱️ Prática com Tempo"],
        )
...
def render_interview_simulation():
    ...
    try:
        ...
        if hasattr(st.session_state, "current_problem") and st.session_state.current_problem:
            ...
            code = st.text_area(
                "Escreva seu código aqui:", height=300, value=problem.get("template", "# Escreva sua solução aqui")
            )
...
def render_problem_library():
    ...
    problems = [
        ...
        },
        {
            ...
            "examples": [{"input": "1->2->3->4->5", "output": "5->4->3->2->1", "explanation": "Inverter a ordem dos nós"}],
        },
        {
            ...
            "examples": [{"input": "[-2,1,-3,4,-1,2,1,-5,4]", "output": "6", "explanation": "Subarray [4,-1,2,1] tem soma 6"}],
        },
        {
    ...
    with col2:
        filter_category = st.selectbox(
            "Filtrar por Categoria", ["Todas", "Array", "String", "Linked List", "Tree", "Dynamic Programming"]
        )
    ...
    if st.button("🔍 Analisar Código"):
        try:
            ...
            has_nested_loops = any(
                "for" in line and any("for" in other for other in lines[i + 1 :]) for i, line in enumerate(lines)
            )
