...
class AdvancedStructuresVisualizer:
    ...
    def __init__(self, figsize=(16, 12)):
        ...
        self.fig.suptitle("🏗️ Advanced Data Structures Visualizer", fontsize=self.config.title_font_size, fontweight="bold")
    ...
    def visualize_heap(self, heap_data: Dict[str, Any]):
        ...
        self.ax_main.set_title(f'🔺 {"Max" if is_max_heap else "Min"} Heap', fontweight="bold")

        if not heap:
            self.ax_main.text(0.5, 0.5, "Heap Vazio", transform=self.ax_main.transAxes, ha="center", va="center", fontsize=14)
            return
        ...
        for i, (x, y) in enumerate(positions):
            if i < len(heap):
                circle = Circle((x, y), 0.3, color=self.config.primary_color, alpha=self.config.normal_alpha)
        ...
        for i in range(len(heap)):
            ...
            if left_child < len(heap):
                ...
                self.ax_main.plot(
                    [x1, x2], [y1, y2], color=self.config.text_color, linewidth=self.config.edge_width, alpha=0.7
                )
    ...
    def visualize_trie(self, trie_data: Dict[str, Any]):
        ...
        if G.number_of_nodes() == 0:
            self.ax_main.text(0.5, 0.5, "Trie Vazia", transform=self.ax_main.transAxes, ha="center", va="center", fontsize=14)
            return
        ...
        nx.draw_networkx_nodes(
            G, pos, ax=self.ax_main, node_color=node_colors, node_size=self.config.node_size, alpha=self.config.normal_alpha
        )

        # Desenhar arestas
        nx.draw_networkx_edges(
            G, pos, ax=self.ax_main, edge_color=self.config.text_color, width=self.config.edge_width, alpha=0.7
        )

        # Adicionar labels
        labels = {node: node.split("_")[-1] if "_" in node else node for node in G.nodes()}
        nx.draw_networkx_labels(G, pos, labels, ax=self.ax_main, font_size=self.config.font_size, font_weight="bold")

        # Adicionar labels das arestas
        edge_labels = nx.get_edge_attributes(G, "char")
        nx.draw_networkx_edge_labels(G, pos, edge_labels, ax=self.ax_main, font_size=self.config.font_size - 2)
    ...
    def visualize_union_find(self, uf_data: Dict[str, Any]):
        ...
        # Adicionar labels
        nx.draw_networkx_labels(G, pos, ax=self.ax_main, font_size=self.config.font_size, font_weight="bold")
    ...
    def visualize_segment_tree(self, seg_tree_data: Dict[str, Any]):
        ...
        for i in range(min(relevant_size, len(tree))):
            if tree[i] != 0:  # Apenas nós com valores
                ...
                circle = Circle((x, y), 0.2, color=self.config.secondary_color, alpha=self.config.normal_alpha)
                self.ax_main.add_patch(circle)
        ...
        for i, val in enumerate(array):
            ...
            rect = Rectangle(
                (x - 0.2, array_y - 0.2), 0.4, 0.4, color=self.config.success_color, alpha=self.config.normal_alpha
            )
            self.ax_main.add_patch(rect)

            self.ax_main.text(
                x, array_y, str(val), ha="center", va="center", fontsize=self.config.font_size, fontweight="bold"
            )
    ...
    def visualize_lru_cache(self, lru_data: Dict[str, Any]):
        ...
        if not order:
            self.ax_main.text(0.5, 0.5, "Cache Vazio", transform=self.ax_main.transAxes, ha="center", va="center", fontsize=14)
            return
    ...
    def _render_heap_info(self, heap_data: Dict[str, Any]):
        ...
        self.ax_info.text(
            ...
            ha="left",
            fontsize=10,
            bbox=dict(boxstyle="round,pad=0.3", facecolor=self.config.background_color),
        )
    ...
    def _render_operations(self, operations: List[Dict[str, Any]]):
        ...
        if not operations:
            self.ax_operations.text(
                0.5, 0.5, "Nenhuma operação", transform=self.ax_operations.transAxes, ha="center", va="center", fontsize=10
            )
        ...
        for i, (op, y) in enumerate(zip(recent_ops, y_positions)):
            ...
            self.ax_operations.text(
                ...
                ha="left",
                fontsize=9,
                bbox=dict(boxstyle="round,pad=0.2", facecolor=self.config.primary_color, alpha=0.3),
            )
    ...
    def render_complexity_analysis(self, structure_type: str, operations: List[str]):
        ...
        complexity_data = {
            "heap": {"insert": "O(log n)", "extract": "O(log n)", "peek": "O(1)", "build": "O(n)"},
            "trie": {"insert": "O(m)", "search": "O(m)", "delete": "O(m)", "prefix": "O(p + n)"},
            "union_find": {"find": "O(α(n))", "union": "O(α(n))", "connected": "O(α(n))"},
            "segment_tree": {"query": "O(log n)", "update": "O(log n)", "build": "O(n)"},
            "lru_cache": {"get": "O(1)", "put": "O(1)"},
            "graph": {"dfs": "O(V + E)", "bfs": "O(V + E)", "add_edge": "O(1)"},
        }
        ...
        complexity_to_value = {"O(1)": 1, "O(log n)": 2, "O(α(n))": 1.5, "O(m)": 3, "O(n)": 4, "O(p + n)": 5, "O(V + E)": 6}

        for op in operations_list:
            complexity_values.append(complexity_to_value.get(complexities[op], 3))

        bars = self.ax_complexity.bar(
            operations_list, complexity_values, color=self.config.primary_color, alpha=self.config.normal_alpha
        )
...
def test_visualizer():
    ...
    trie_data = {"tree_state": trie._get_tree_state(), "operations": trie.operations}
    ...
    uf_data = {"parent_state": uf.parent, "rank_state": uf.rank, "components": uf.components, "operations": uf.operations}
