...
def bubble_sort_com_passos(arr):
    ...
    for i in range(n):
        for j in range(0, n - i - 1):
            ...
            passos.append({"array": arr_copy.copy(), "comparando": [j, j + 1], "i": i, "j": j})

            if arr_copy[j] > arr_copy[j + 1]:
                arr_copy[j], arr_copy[j + 1] = arr_copy[j + 1], arr_copy[j]

    passos.append({"array": arr_copy.copy(), "comparando": [], "final": True})
...
def quick_sort_com_passos(arr, start=0, end=None):
    ...
    def partition(arr, low, high, passos):
        ...
        passos.append({"array": arr.copy(), "pivot": high, "range": [low, high], "action": f"Escolhendo pivot: {pivot}"})
        ...
        for j in range(low, high):
            if arr[j] <= pivot:
                ...
                passos.append(
                    {"array": arr.copy(), "pivot": high, "trocou": [i, j], "action": f"Trocou {arr[j]} com {arr[i]}"}
                )

        arr[i + 1], arr[high] = arr[high], arr[i + 1]
        passos.append({"array": arr.copy(), "pivot_final": i + 1, "action": f"Pivot {pivot} na posição final"})
...
def quick_sort_steps(arr_tuple):
    ...
    passos.append(
        {"pivot": pivot, "left": list(left_sorted), "right": list(right_sorted), "action": f"Pivot {pivot} aplicado"}
    )
...
def merge_sort_com_passos(arr):
    ...
    def merge(left, right, start_idx):
        ...
        passos.append(
            {"merging": True, "left": left, "right": right, "start_idx": start_idx, "action": f"Mergeando {left} e {right}"}
        )
        ...
        passos.append({"merged": result, "start_idx": start_idx, "action": f"Resultado: {result}"})
...
def heap_sort_com_passos(arr):
    ...
    passos.append({"array": arr_copy.copy(), "heap_construido": True, "action": "Max heap construído"})

    # Extrair elementos
    for i in range(n - 1, 0, -1):
        arr_copy[0], arr_copy[i] = arr_copy[i], arr_copy[0]
        passos.append(
            {"array": arr_copy.copy(), "extraiu": arr_copy[i], "heap_size": i, "action": f"Extraiu máximo: {arr_copy[i]}"}
        )
...
def counting_sort_com_passos(arr):
    ...
    passos.append({"count_array": count.copy(), "action": f"Contagem: {count}"})
    ...
    passos.append({"resultado": result, "action": f"Array ordenado: {result}"})
