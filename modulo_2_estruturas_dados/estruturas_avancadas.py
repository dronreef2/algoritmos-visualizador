...
class AdvancedHeap:
    ...
    def _heapify_up(self, index: int):
        ...
        if index > 0 and self._compare(self.heap[index], self.heap[parent_index]):
            ...
            self._log_operation('heapify_up', {
                'swapped_indices': [index, parent_index],
                'swapped_values': [self.heap[parent_index], self.heap[index]]
            })
    ...
    def _heapify_down(self, index: int):
        ...
        if (left_child < size and 
            self._compare(self.heap[left_child], self.heap[largest])):
            largest = left_child
        
        if (right_child < size and 
            self._compare(self.heap[right_child], self.heap[largest])):
            largest = right_child
        
        if largest != index:
            self.heap[index], self.heap[largest] = self.heap[largest], self.heap[index]
            self._log_operation('heapify_down', {
                'swapped_indices': [index, largest],
                'swapped_values': [self.heap[largest], self.heap[index]]
            })
    ...
    def extract_root(self) -> Optional[int]:
        ...
        if len(self.heap) == 1:
            root = self.heap.pop()
            self._log_operation('extract_root', {
    ...
    def build_heap(self, arr: List[int]):
        ...
        """
        self.heap = arr.copy()
        self._log_operation('build_heap', {
...
class Trie:
    ...
    def search(self, word: str) -> bool:
        ...
        self._log_operation('search', {
            ...
            'path': path,
            'found': found,
            'word_count': node.word_count if found else 0
        })
    ...
    def starts_with(self, prefix: str) -> bool:
        ...
        for char in prefix:
            path.append(char)
            if char not in node.children:
                self._log_operation('starts_with', {
                    'prefix': prefix,
                    'path': path,
                    'found': False
                })
    ...
    def get_words_with_prefix(self, prefix: str) -> List[str]:
        ...
        self._log_operation('get_words_with_prefix', {
            'prefix': prefix,
            'words_found': words,
            'count': len(words)
        })
    ...
    def _get_tree_state(self) -> Dict[str, Any]:
        ...
        def _serialize_node(node: TrieNode) -> Dict[str, Any]:
            return {
                ...
                'children': {char: _serialize_node(child) 
...
class SegmentTree:
    ...
    def range_update(self, left: int, right: int, value: int):
        ...
        self._log_operation('range_update', {
            'left': left,
            'right': right,
            'value': value
        })
    
    def _range_update_helper(self, node: int, start: int, end: int, 
        ...
        if key in self.cache:
            ...
            if len(self.cache) >= self.capacity:
                ...
                self._log_operation('put', {
                    'key': key,
                    'value': value,
                    'action': 'add_with_eviction',
                    'evicted_key': tail_node.key
                })
            else:
                self._log_operation('put', {
                    'key': key,
...
class Graph:
    ...
    def dfs(self, start: int, visited: Optional[Set[int]] = None) -> List[int]:
        ...
        self._log_operation('dfs', {
            'start': start,
            'path': path,
            'visited_order': result
        })
    ...
    def connected_components(self) -> List[List[int]]:
        ...
        self._log_operation('connected_components', {
            'components': components,
