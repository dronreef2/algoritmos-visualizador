...
class InterviewVisualizer:
    ...
    def visualize_problem(self, problem_id: str, steps: List[Dict[str, Any]], 
    ...
    def _render_two_sum(self, steps: List[Dict], test_results: Dict = None, 
        ...
        # Renderizar array
        bars = self.ax_problem.bar(range(len(nums)), nums, 
                                  color=self.colors['primary'], alpha=0.7)
        
        # Adicionar valores no topo das barras
        for i, (bar, num) in enumerate(zip(bars, nums)):
            self.ax_problem.text(bar.get_x() + bar.get_width()/2, 
                               bar.get_height() + 0.1, 
                               f'{num}\n[{i}]', 
                               ha='center', va='bottom', 
                               fontweight='bold')
        ...
        if len(steps) > 1:
            ...
            if 'hash_map' in last_step:
                ...
                self.ax_problem.text(0.02, 0.98, hash_text, 
    ...
    def _render_valid_parentheses(self, steps: List[Dict], test_results: Dict = None, 
                                 code_analysis: Dict = None):
        ...
        bars = self.ax_problem.bar(char_positions, char_heights, 
                                  color=char_colors, alpha=0.7)
        ...
        if len(steps) > 1:
            ...
            if 'stack' in last_step:
                ...
                self.ax_problem.text(0.02, 0.98, stack_text, 
                                   transform=self.ax_problem.transAxes,
                                   verticalalignment='top',
                                   family='monospace',
                                   bbox=dict(boxstyle="round,pad=0.3", 
    ...
    def _render_generic_problem(self, steps: List[Dict], test_results: Dict = None, 
                               code_analysis: Dict = None):
        ...
        if steps:
            ...
            self.ax_problem.text(0.5, 0.5, info_text, 
                               transform=self.ax_problem.transAxes,
                               ha='center', va='center',
                               fontsize=12,
    ...
    def _render_algorithm_progress(self, steps: List[Dict]):
        ...
        # Plot timeline
        self.ax_progress.scatter(step_numbers, [1] * len(steps), 
                               c=colors, s=100, alpha=0.7)
        
        # Conectar pontos
        self.ax_progress.plot(step_numbers, [1] * len(steps), 
                            color='gray', alpha=0.3, linewidth=2)
        
        # Adicionar labels
        for i, step_type in enumerate(step_types):
            self.ax_progress.annotate(step_type, 
                                    (i, 1), 
                                    xytext=(0, 20), 
    ...
    def _render_test_results(self, test_results: Dict):
        ...
        if total > 0:
            wedges, texts, autotexts = self.ax_complexity.pie(sizes, labels=labels, colors=colors,
                                                            autopct=lambda pct: f'{int(pct/100 * total)}',
                                                            startangle=90)
            
            # Adicionar informações
            self.ax_complexity.text(0, -1.3, f'Taxa de Sucesso: {passed}/{total} ({passed/total*100:.1f}%)',
                                  ha='center', va='center', 
                                  fontsize=10, fontweight='bold')
            
            # Tempo de execução
            exec_time = test_results.get('execution_time', 0)
            self.ax_complexity.text(0, -1.5, f'Tempo: {exec_time:.4f}s',
                                  ha='center', va='center', 
                                  fontsize=9)
    ...
    def _render_code_analysis(self, code_analysis: Dict):
        ...
        bars = self.ax_score.barh(metrics_names, metrics_values, 
                                color=[self.colors['primary'], self.colors['secondary'], 
                                      self.colors['success'], self.colors['warning']])
        
        # Adicionar valores nas barras
        for bar, value in zip(bars, metrics_values):
            self.ax_score.text(bar.get_width() + 1, bar.get_y() + bar.get_height()/2, 
                             f'{value:.0f}', 
        ...
        if patterns:
            ...
            self.ax_score.text(0.02, 0.02, patterns_text, 
    ...
    def _complexity_to_score(self, complexity: str) -> int:
        ...
        complexity_scores = {
            'O(1)': 100, 'O(log n)': 90, 'O(n)': 80,
            'O(n log n)': 70, 'O(n²)': 50, 'O(2^n)': 20
        }
    ...
    def _update_two_sum_frame(self, step: Dict):
        ...
        # Redesenhar barras
        colors = ['red' if i == current_index else self.colors['primary'] 
    ...
    def _update_valid_parentheses_frame(self, step: Dict):
        ...
        bars = self.ax_problem.bar(range(len(string)), [1] * len(string), 
...
class StreamlitRenderer:
    ...
    def render_problem_analysis(self, problem_id: str, user_code: str, 
    ...
    def render_comparison(self, user_solution: str, optimal_solution: str, 
                         problem_id: str) -> Dict[str, Any]:
        """
