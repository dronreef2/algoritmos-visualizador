...
class TestRunner:
    ...
    def run_user_code(self, user_code: str, problem: Problem) -> Dict[str, Any]:
        ...
        """
        results = {"passed": 0, "total": len(problem.test_cases), "test_results": [], "error": None, "execution_time": 0}
...
class CodeAnalyzer:
    ...
    def _estimate_complexity(self, code: str) -> Dict[str, str]:
        ...
        return {"time": time_complexity, "space": "O(1)" if not has_recursion else "O(n)"}
    ...
    def _calculate_score(self, analysis: Dict[str, Any]) -> int:
        ...
        complexity_scores = {"O(1)": 30, "O(log n)": 25, "O(n)": 20, "O(n log n)": 15, "O(n²)": 10, "O(2^n)": 5}
...
class SolutionVisualizer:
    ...
    def _generate_valid_parentheses_steps(self, s: str) -> List[Dict[str, Any]]:
        ...
        steps.append(
            {"tipo": "inicio", "string": s, "stack": stack.copy(), "current_index": -1, "action": f'Verificando string "{s}"'}
        )
...
class InterviewSession:
    ...
    def get_random_problem(self, difficulty: str = "Medium", category: str = "Array"):
        ...
        difficulty_map = {"Easy": ["two_sum"], "Medium": ["two_sum", "valid_parentheses"], "Hard": ["reverse_linked_list"]}
